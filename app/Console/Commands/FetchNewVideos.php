...
class FetchNewVideos extends Command
{
     ...
     * @var string
     */
    protected $signature = 'fetch:new-videos';
     ...
     * @var string
     */
    protected $description = 'Fetches new videos';
    ...
    public function handle()
    {
        ...
        $week_number = VideoStat::latest()->first()->week_number;
        foreach ($channels as $key => $channel) {
            ...
            do {
                ...
                foreach ($playlist_items_response->items as $item) {
                    ...
                    $all_videos[] = $item->snippet->resourceId->videoId;
                }
                ...
                if ($existing_videos) {
                    ...
                    foreach ($responses_existing as $res) {
                        ...
                        $video->view_count = $res->statistics->viewCount ?? 0;
                        $video->update();
                        $before_date = Carbon::now()->subWeek();
            ...
            $deleted_videos = array_diff($old_videos, $all_videos);
            if ($deleted_videos) {
                foreach ($deleted_videos as $vid) {
                    $video = Video::where(['channel_id' => $channel->id, 'video_id' => $vid])->first();
                    $video->delete();
                }
            }
        }
